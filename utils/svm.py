...
def read_sample(path):
    with open(path) as f:
        ...
        data_ = a.split('\n')[0].split(' ')[:-1]
        label_ = a.split('\n')[1].split(' ')[:-1]
        data,label = zeros(len(data_)),zeros(len(label_))
...
def main():
    ...
    data, label = read_sample('D:/sweetvegetable/data.txt')
    for i in range(len(label)):
        if label[i] == 0:
            label[i] = -1
    print (data)
    print (label)
    b,alpha = smoSimple(data,label,0.6,0.001,40)   # C toler maxIter
    print (b)
    print (alpha)
    for i in range(100):
        if alpha[i]>0:
